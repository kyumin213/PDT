<template>
	<view class="containerBox">
		<tab-top left-icon="back" :title="languageInfo.payPwdModify"></tab-top>
		<view class="content">
			<view class="iconBox">
				<image src="@/static/me/icon9.png" class="icon11"></image>
			</view>
			<view class="itemBox">
				<view class="items">
					<image src="@/static/index/icon12.png" class="icon12"></image>
					<text class="itemTxt">{{languageInfo.oldPayPwd}}</text>
					<image src="@/static/index/icon12.png" class="icon12"></image>
				</view>
				<view>
					<input type="password" @input="changeLogoPwd(oldPwd)" :placeholder="languageInfo.placeOldPayPwd" class="inp" v-model="oldPwd">
				</view>
				<view class="items">
					<image src="@/static/index/icon12.png" class="icon12"></image>
				...
				<view class="tipBox">
					...
					<text class="tips" v-show="confirNewPwd!=''&&confirNewPwd!=newPwd">{{languageInfo.twoPwd}}</text>
				</view>
				<view>
		...
		methods: {
			...
			confirmBtn(){
				...
				if(old==''|| pwd==''||pwds==''){
					...
					_this.post('api/changePaymentPassword',data).then(res=>{
						uni.showToast({
							icon:'success'
						})
						setTimeout(()=>{
