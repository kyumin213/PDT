<template>
	<view class="containerBox">
		<tab-top left-icon="back" :title="languageInfo.rechargeRecord"></tab-top>
		<view class="content">
			<view class="topBox">
				<view class="setItem">
					<image src="@/static/index/icon38.png" class="icon36"></image>
					<view class="setItemBox">
						<text>{{languageInfo.rechargeRecord}}</text>
					</view>
				</view>
			</view>
			<!-- 列表 -->
			<view class="listBox">
				...
				<scroll-view scroll-y="true" class="scrollHeight" @scrolltolower="getmore">
					<view class="listItem" v-for="(item,index) in lists" :key="index">
						...
						<text class="items" v-if="item.currency_id=='4'">+{{item.num}} USDT</text>
					</view>
					<view class="tipTxt" v-if="lists.length==0">{{languageInfo.noData}}</view>
					<view class="loading-text" v-if="loadingText==1">{{languageInfo.theEnd}}</view>
				</scroll-view>
			</view>
		</view>
	</view>
